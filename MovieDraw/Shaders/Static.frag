...
void main()
{
	...
	vec4 staticColor = texture(lightmapTexture, lightmapCoords);
	vec4 totalColor = vec4(0, 0, 0, 0);
	
	#pragma insert ShadowFragLight
	
	// Include vertex and object colors only when doing nonstatic lighting (which is currently disabled)
	finalColor = vec4(((totalColor * color2 * vertexColor) + (color2 * staticColor)).rgb, color2.a);
}
