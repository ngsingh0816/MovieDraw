...
{
	return [ [ MDProgressBar alloc ] init ];
}

+ (MDProgressBar*) mdProgressBarWithFrame: (MDRect)rect background:(NSColor*)bkg
{
	return [ [ MDProgressBar alloc ] initWithFrame:rect background:bkg ];
}
...
- (instancetype) init
{
	if ((self = [ super init ]))
	{
		background = MD_PROGRESS_DEFAULT_BKG2;
		minValue = 0;
		maxValue = 100;
		currentValue = 0;
		otherColor = [ NSColor greenColor ];
		rotation = 0;
		speed = 12.5;
		width1 = 20;
		width2 = 20;
...
- (void) setType: (MDProgressBarType)ty
{
	...
	if (type == MD_PROGRESSBAR_SPIN)
	{
		if ([ otherColor isEqualTo:[ NSColor greenColor ] ])
			otherColor = [ NSColor colorWithCalibratedRed:0 green:0 blue:0 alpha:0 ];
	}
	changed = TRUE;
}
...
- (void) drawView
{
	...
	else
	{
		...
		/*float diffRed = 0;//([background[0] redComponent ] - [ background[1] redComponent ]) / 11;
		float diffGreen = 0;//([background[0] greenComponent ] - [ background[1] greenComponent ]) / 11;
		float diffBlue = 0;//([background[0] blueComponent ] - [ background[1] blueComponent ]) / 11;
		float diffAlpha = 0;//([background[0] alphaComponent ] - [ background[1] alphaComponent ]) / 11;*/
