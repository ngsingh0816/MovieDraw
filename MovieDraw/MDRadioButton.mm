...
void CreateRadioGroup(NSArray* members)
{
	for (int z = 0; z < [ members count ] - 1; z++)
	{
		for (int y = z + 1; y < [ members count ]; y++)
			[ (MDRadioButton*)members[z] addMember:members[y] ];
	}
}
...
{
	MDRadioButton* view = [ [ MDRadioButton alloc ] init ];
	return view;
}

+ (instancetype) mdRadioButtonWithFrame: (MDRect)rect background: (NSColor*)bkg;
{
	MDRadioButton* view = [ [ MDRadioButton alloc ] initWithFrame:rect background:bkg ];
	return view;
}
...
{
	members = [ [ NSMutableArray alloc ] initWithArray:mem ];
}
		 ...
		 point.y >= frame.y && point.y <= frame.y + frame.height) || distanceB(NSMakePoint(frame.x + (frame.width / 2), frame.y + (frame.height / 2)), point) <= (frame.width / 2) + 1)
	{
		...
		if (down)
		{
			...
			for (int z = 0; z < [ members count ]; z++)
				[ (MDControl*)members[z] setState:0 ];
			changed = TRUE;
		}
	}
	if (down)
...
- (void) drawView
{
	...
	if (changed)
	{
		...
		for (int z = 0; z < 38; z++)
		{
			...
			NSColor* bcolor = MD_BUTTON_DEFAULT_BUTTON_COLOR;//MD_BUTTON_DEFAULT_BORDER_COLOR;
			//if (state)
