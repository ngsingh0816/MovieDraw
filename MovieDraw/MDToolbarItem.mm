...
{
	MDToolbarItem* view = [ [ MDToolbarItem alloc ] init ];
	return view;
}

+ (instancetype) mdToolbarItemWithFrame: (MDRect)rect background: (NSColor*)bkg
{
	MDToolbarItem* view = [ [ MDToolbarItem alloc ] initWithFrame:rect background:bkg ];
	return view;
}
...
- (instancetype) init
{
	if ((self = [ super init ]))
	{
		...
		type = MDITEM_NORMAL;
		img = 0;
		textFont = [ NSFont systemFontOfSize:10 ];
		overlay = [ NSColor colorWithCalibratedRed:1 green:1 blue:1 alpha:1 ];
	}
	return self;
}
...
- (void) setImagePath: (char*)data length:(unsigned int)len;
{
	...
	if (len == 0)
	{
		...
		if ([ path hasPrefix:@"NS" ])
		{
			...
			if (timage)
			{
				NSData* data = [ timage TIFFRepresentation ];
		...
		LoadImage([ image UTF8String ], &img, 0);
	}
	else
	{
...
- (void) drawView
{
	...
	if ((image && [ image length ] != 0) || img != 0)
	{
		...
		glColor4f([ overlay redComponent ] + add, [ overlay greenComponent ] + add,
				  [ overlay blueComponent ] + add, [ background alphaComponent ]);
