...
{
	return [ [ MDComboBox alloc ] init ];
}

+ (MDComboBox*) mdComboBoxWithFrame:(MDRect)rect background:(NSColor*)bkg
{
	return [ [ MDComboBox alloc ] initWithFrame:rect background:bkg ];
}
...
- (void) downPressed
{
	...
	{
		[ array addObject:[ MDMenuItem menuItemWithString:[ [ (GLString*)strings[z] string ] string ] target:self action:@selector(itemChosen:) ] ];
	}
	
	popUp = MDCreatePopupMenu(array, NSMakePoint(frame.x, frame.y - 7), frame.width);
...
- (void) selectItem: (unsigned long)item
{
	if (item >= [ titems count ])
		return;
	[ field setText:titems[item] ];
	text = [ [ NSMutableString alloc ] initWithString:titems[item] ];
