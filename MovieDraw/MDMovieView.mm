...
{
	return [ [ MDMovieView alloc ] init ];
}

+ (instancetype) mdMovieViewWithFrame: (MDRect)rect background:(NSColor*)bkg
{
	return [ [ MDMovieView alloc ] initWithFrame:rect background:bkg ];
}
...
- (instancetype) initWithFrame:(MDRect)rect background:(NSColor *)bkg
{
	if ((self = [ super initWithFrame:rect background:bkg ]))
	{
		...
		//[ progress setType:MD_PROGRESSBAR_CIRCLE ];
...
- (BOOL) loadMovie: (NSString*)path
{
	...
	{
		movieName = [ [ NSString alloc ] initWithFormat:@"%@/%@", [ [ NSBundle mainBundle ] resourcePath ], path ];
	}
...
- (void) drawView
{
		...
		
	{
		...
		glBindTexture(GL_TEXTURE_2D, [ textures[currentFrame] unsignedIntValue ]);
		glColor4d(1, 1, 1, 1);
		glBegin(GL_QUADS);
		{
			glTexCoord2d(0, 0);
...
- (void) update
{
	...
	if ((float)time >= [ slider maxValue ])
	{
		if (finish && [ finish respondsToSelector:finishAct ])
			((void (*)(id, SEL))[ finish methodForSelector:finishAct ])(finish, finishAct);
		[ self stop:self ];
	}
}
...
- (void) dealloc
{
	...
	if (textures)
	{
		...
		{
			unsigned int image = [ textures[z] unsignedIntValue ];
			ReleaseImage(&image);
		}
	}
}
