...
{
	return [ [ [ MDTextField alloc ] init ] autorelease ];
}

+ (MDTextField*) mdTextFieldWithFrame:(MDRect) rect background:(NSColor*)bkg
{
	return [ [ [ MDTextField alloc ] initWithFrame:rect background:bkg ] autorelease ];
}
...
- (void) drawView
{
	...
	for (int z = 0; z < 4; z++)
	{
		colors[(z * 4)] = [ background[z] redComponent ];
		colors[(z * 4) + 1] = [ background[z] greenComponent ];
		colors[(z * 4) + 2] = [ background[z] blueComponent ];
	...
	for (unsigned long z = 0; z < textLength; z++)
	{
		...
		GLString* string = [ characters objectAtIndex:z ];
		if (safeText)
			string = safeText;
...
- (void) keyDown: (NSEvent*) theEvent
{
		...
		case NSDeleteCharacter:
			if (editable)
			{
				if ([ self hasHighlights ])
				{
					...
					do
					{
						if (lastIndex == NSNotFound)
							break;
						scrollx -= [ [ characters objectAtIndex:lastIndex ] frameSize ].width;
						if (scrollx < 0)
							scrollx = 0;
				...
				else if (cursorIndex != 0)
				{
					cursorIndex--;
					scrollIndex--;
					scrollx -= [ [ characters objectAtIndex:cursorIndex ] frameSize ].width;
					if (scrollx < 0)
						scrollx = 0;
	...
	if (keyTar && [ keyTar respondsToSelector:keyAct ])
		[ keyTar performSelector:keyAct withObject:self ];
}
