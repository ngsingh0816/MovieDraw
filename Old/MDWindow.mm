...
+ (id) mdWindow
{
	MDWindow* view = [ [ [ MDWindow alloc ] init ] autorelease ];
	return view;
}

+ (id) mdWindowWithFrame: (MDRect)rect background: (NSColor*)bkg;
{
	MDWindow* view = [ [ [ MDWindow alloc ] initWithFrame:rect
...
+ (void) choiceChosen: (id) sender
{
	...
	if ([ [ (MDButton*)sender identity ] isEqualToString:@"Default" ])
		...
		number = [ NSNumber numberWithUnsignedInt:1 ];
	else
		number = [ NSNumber numberWithUnsignedInt:2 ];
...
+ (void) zoomIntro: (NSTimer*)timer
{
	...
	NSColor* color = [ currentAlert backgroundAtIndex:0 ];
	...
	{
		if ([ [ (MDControlView*)[ [ currentAlert subViews ] objectAtIndex:z ] identity ] isEqualToString:@"Title" ])
		{
			MDLabel* view = [ [ currentAlert subViews ] objectAtIndex:z ];
			color = [ view backgroundAtIndex:0 ];
		...
		else if ([ [ (MDControlView*)[ [ currentAlert subViews ] objectAtIndex:z ] identity ] isEqualToString:@"Message" ])
		{
			MDLabel* view = [ [ currentAlert subViews ] objectAtIndex:z ];
			MDLabel* view2 = nil;
			for (int y = 0; y < [ [ currentAlert subViews ] count ]; y++)
			{
				if ([ [ (MDControlView*)[ [ currentAlert subViews ] objectAtIndex:y ] identity ] isEqualToString:@"Title" ])
					view2 = [ [ currentAlert subViews ] objectAtIndex:y ];
			}
		...
		else if ([ [ (MDControlView*)[ [ currentAlert subViews ] objectAtIndex:z ] identity ] isEqualToString:@"Image" ])
		{
			MDImageView* view = [ [ currentAlert subViews ] objectAtIndex:z ];
			color = [ view backgroundAtIndex:0 ];
			[ view setBackground:[ NSColor colorWithCalibratedRed:[ color redComponent ] green:[ color greenComponent ] blue:[ color blueComponent ] alpha:times / (quick - 1) ] ];
			float windowX = (resolution.width - (targetSize.width * times / (quick - 1))) / 2;
			float windowY = (resolution.height - (targetSize.height * times / (quick - 1))) / 2;
		...
		if (![ [ [ currentAlert subViews ] objectAtIndex:z ] isKindOfClass:[ MDButton class ] ])
			continue;
		MDButton* button = [ [ currentAlert subViews ] objectAtIndex:z ];
		if ([ [ button identity ] isEqualToString:@"Default" ])
		{
			...
			color = [ button backgroundAtIndex:0 ];
			[ button setBackground:[ NSColor colorWithCalibratedRed:[ color redComponent ] green:[ color greenComponent ] blue:[ color blueComponent ] alpha:times / (quick - 1) ] atIndex:0 ];
...
{	
	...
	{
		MDRect zf = [ (MDControlView*)[ subViews objectAtIndex:z ] frame ];
		if (!resizeViews)
		{
			[ (MDControlView*)[ subViews objectAtIndex:z ] setFrame:
	...
	if (resizeTar && [ resizeTar respondsToSelector:resizeAct ])
		[ resizeTar performSelector:resizeAct withObject:self ];
}
...
- (void) mouseDown:(NSEvent*)event
{
	...
	for (int z = 0; z < [ subViews count ]; z++)
		[ [ subViews objectAtIndex:z ] mouseDown:event ];
...
- (void) mouseDragged:(NSEvent*)event
{
	...
	for (int z = 0; z < [ subViews count ]; z++)
		[ [ subViews objectAtIndex:z ] mouseDragged:event ];
...
- (void) mouseUp:(NSEvent*)event
{
	...
	for (int z = 0; z < [ subViews count ]; z++)
		[ [ subViews objectAtIndex:z ] mouseUp:event ];
}
...
- (void) mouseMoved:(NSEvent*)event
{
	...
	for (int z = 0; z < [ subViews count ]; z++)
		[ [ subViews objectAtIndex:z ] mouseMoved:event ];
	resizing = FALSE;
}
...
- (void) scrollWheel:(NSEvent *)event
{
	...
	{
		[ [ subViews objectAtIndex:z ] scrollWheel:event ];
		if ([ [ subViews objectAtIndex:z ] scrolled ])
		{
			scrolled = TRUE;
			break;
		}
...
- (void) keyDown:(NSEvent *)event
{
	...
	for (int z = 0; z < [ subViews count ]; z++)
		[ [ subViews objectAtIndex:z ] keyDown:event ];
}
...
- (void) keyUp:(NSEvent *)event
{
	...
	for (int z = 0; z < [ subViews count ]; z++)
		[ [ subViews objectAtIndex:z ] keyUp:event ];
}
...
- (void) drawView
{
	...
	if (titleChanged)
	{
		...
		for (int z = 0; z < 22; z++)
		{
			...
			titleColors[(z * 4) + 3] = [ background[0] alphaComponent ];
		}
...
#define INTENSITY	0
	...
	{
		NSColor* color = MD_WINDOW_DEFAULT_TITLE_COLOR3;
		glColor4d([ color redComponent ], [ color greenComponent ], [ color blueComponent ], [ background[0] alphaComponent ]);
		glVertex2d(3.5, 0);
	...
	glBegin(GL_QUADS);
	{
		...
		glColor4d(0, 0, 0, alpha * [ background[0] alphaComponent ]);
		glVertex2d(frame.width, -TITLE_HEIGHT);
		glVertex2d(0, -TITLE_HEIGHT);
	}
	glEnd();
	...
	for (int z = 0; z < [ subViews count ]; z++)
		[ [ subViews objectAtIndex:z ] drawView ];
...
- (void) close: (id) sender
{
	if (target && [ target respondsToSelector:action ])
		[ target performSelector:action withObject:(sender ? sender : self) ];
...
- (void) removeSubViewAtIndex: (unsigned int) index
{
	if (index >= [ subViews count ])
		return;
	[ self removeSubView:[ subViews objectAtIndex:index ] ];
}
...
- (void) dealloc
{
	...
	if (subViews)
	{
		for (int z = 0; z < [ subViews count ]; z++)
		{
			if ([ views containsObject:[ subViews objectAtIndex:z ] ])
				[ views removeObject:[ subViews objectAtIndex:z ] ];
		}
		[ subViews removeAllObjects ];
		subViews = nil;
	}
