...
+ (id) mdToolbar
{
	MDToolbar* view = [ [ [ MDToolbar alloc ] init ] autorelease ];
	return view;
}

+ (id) mdToolbarWithFrame: (MDRect)rect background: (NSColor*)bkg
{
	MDToolbar* view = [ [ [ MDToolbar alloc ] initWithFrame:rect
...
- (void) setShown: (BOOL) set
{
	...
	for (int z = 0; z < [ titems count ]; z++)
	{
		if (shown)
		{
			if ([ (MDControlView*)[ titems objectAtIndex:z ] frame ].x +
			[ (MDControlView*)[ titems objectAtIndex:z ] frame ].width >= 
			frame.x + frame.width - (frame.height / 2))
				break;
		}
		[ [ titems objectAtIndex:z ] setVisible:shown ];
	}
	if (!shown)
		flags &= ~(1 | (1 << 1));
}
...
- (void) setBackground: (NSColor*)bkg atIndex:(unsigned int)index
{
	...
	{
		for (int z = 0; z < [ titems count ]; z++)
			[ [ titems objectAtIndex:z ] setBackground:bkg atIndex:index ];
	}
}
...
- (void) setRed: (float)red
{
	[ super setRed:red ];
	for (int z = 0; z < [ titems count ]; z++)
		[ [ titems objectAtIndex:z ] setRed:red ];
}
...
- (void) setGreen: (float)green
{
	...
	for (int z = 0; z < [ titems count ]; z++)
		[ [ titems objectAtIndex:z ] setGreen:green ];
}
...
- (void) setBlue: (float)blue
{
	[ super setBlue:blue ];
	for (int z = 0; z < [ titems count ]; z++)
		[ [ titems objectAtIndex:z ] setBlue:blue ];
}
...
- (void) setAlpha: (float)alpha
{
	...
	for (int z = 0; z < [ titems count ]; z++)
		[ (MDToolbarItem*)[ titems objectAtIndex:z ] setAlpha:alpha ];
}
		...
		point.y >= frame.y && point.y <= frame.y + frame.height) == should)
	{
		...
		if (titems && changing != 1)
		{
			for (int z = 0; z < [ titems count ]; z++)
			{
				...
				{
					if ([ (MDControlView*)[ titems objectAtIndex:z ] frame ].x +
					[ (MDControlView*)[ titems objectAtIndex:z ] frame ].width >= fakeFrame.x)
						break;
				}
				[ [ titems objectAtIndex:z ] setVisible:shown ];
			}
...
- (void) mouseDragged:(NSEvent*)event
{
	...
	for (int z = 0; z < [ titems count ]; z++)
		width += [ (MDToolbarItem*)[ titems objectAtIndex:z ] frame ].width;
	...
	else if (([ (MDToolbarItem*)[ titems objectAtIndex:0 ] frame ].x > frame.x + 10)
			 && translate < 0 && [ (MDToolbarItem*)[ titems objectAtIndex:0 ] visible ])
	{
		lastMouse = point;
		down = FALSE;
		return;
	...
	for (int z = 0; z < [ titems count ]; z++)
	{
		MDRect rect = [ (MDToolbarItem*)[ titems objectAtIndex:z ] frame ];
		
		rect.x -= translate;
		[ (MDToolbarItem*)[ titems objectAtIndex:z ] setFrame:rect ];
		if (rect.x + rect.width >= frame.x + frame.width - (drawTri ? 20 : 0) ||
			rect.x + rect.width < frame.x)
			[ (MDToolbarItem*)[ titems objectAtIndex:z ] setVisible:NO ];
		else
			[ (MDToolbarItem*)[ titems objectAtIndex:z ] setVisible:YES ];
	}
}
...
- (void) setEnabled: (BOOL) en
{
	[ super setEnabled:en ];
	for (int z = 0; z < [ titems count ]; z++)
		[ (MDToolbarItem*)[ titems objectAtIndex:z ] setEnabled:en ];
}
...
- (void) drawView
{
	...
	else if (changing == 0)
	{
		...
		if (up)
		{
			...
			{
				if ([ (MDToolbarItem*)[ titems objectAtIndex:y ] frame ].x < frame.x + speed
					&& [ (MDToolbarItem*)[ titems objectAtIndex:y ] visible ])
				{
					float width = 0;
					for (int z = 0; z < [ titems count ]; z++)
					{
						width += [ (MDToolbarItem*)[ titems objectAtIndex:z ] 
								   frame ].width;
					}
			...
			if ([ titems count ] != 0 && [ (MDToolbarItem*)
						[ titems objectAtIndex:0 ] frame ].x > frame.x + speed)
			{
				for (int z = 0; z < [ titems count ]; z++)
				{
					MDRect rect = [ (MDToolbarItem*)[ titems objectAtIndex:z ] frame ];
		...
		if (drawTri)
		{
			...
			{
				float add = !enabled ? -0.3 : 0.0;
				colors[(z * 4)] = [ background[z] redComponent ] + 0.07 + add;
				colors[(z * 4) + 1] = [ background[z] greenComponent ] + 0.07 + add;
				colors[(z * 4) + 2] = [ background[z] blueComponent ] + 0.07 + add;
				colors[(z * 4) + 3] = [ background[z] alphaComponent ];
			}
	...
	else if (changing != 0)
	{
		...
		{
			float add = !enabled ? -0.3 : 0.0;
			tcolors[(z * 4)] = [ background[z] redComponent ] + add;
			tcolors[(z * 4) + 1] = [ background[z] greenComponent ] + add;
			tcolors[(z * 4) + 2] = [ background[z] blueComponent ] + add;
			tcolors[(z * 4) + 3] = [ background[z] alphaComponent ];
		}
