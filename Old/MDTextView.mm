...
+ (id) mdTextView
{
	return [ [ [ MDTextView alloc ] init ] autorelease ];
}

+ (id) mdTextViewWithFrame: (MDRect)rect background:(NSColor*)bkg
{
	return [ [ [ MDTextView alloc ] initWithFrame:rect background:bkg ] autorelease ];
}
...
{	
	...
	{
		height = [ [ characters objectAtIndex:0 ] frameSize ].height / 2;
		halfHeight = height;
		foundCursor = FALSE;
	}
	float width = 5;
