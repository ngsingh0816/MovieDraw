...
+ (id) mdColorWell
{
	MDColorWell* view = [ [ [ MDColorWell alloc ] init ] autorelease ];
	return view;
}

+ (id) mdColorWellWithFrame: (MDRect)rect background: (NSColor*)bkg
{
	MDColorWell* view = [ [ [ MDColorWell alloc ] initWithFrame:rect 
...
- (id) init
{
	if ((self = [ super init ]))
	{
		...
		selectedColor = [ [ NSColor colorWithDeviceRed:1 green:1 blue:1 alpha:1 ] retain ];
		sliderPos = 1.0;
		update = false;
		wantSel = false;
...
- (void) alphaUpdated: (id) sender
{
	...
	selectedColor = [ [ NSColor colorWithCalibratedRed:red green:green blue:blue alpha:
					   [ (MDSlider*)sender value ] / 100.0 ] retain ];
	if (target)
...
- (void) mouseDown:(NSEvent*)event
{
	...
	if (x < (rect.width / 2.0) - 1)
	{
		...
		if (selectedColor)
			[ selectedColor release ];
		selectedColor = [ [ NSColor colorWithDeviceRed:d0 green:d1 blue:d2
							alpha:[ alphaSlider value ] / 100.0 ] retain ];
		colorPoint = point;
...
{	
	...
	selcol[3] = [ background[0] alphaComponent ] * 255;
	selcol[7] = [ background[1] alphaComponent ] * 255;
	selcol[11] = [ background[2] alphaComponent ] * 255;
	selcol[15] = [ background[3] alphaComponent ] * 255;
	...
	if (update && enabled)
	{
		...
		if (selectedColor)
			[ selectedColor release ];
		selectedColor = [ [ NSColor colorWithDeviceRed:data[0] green:data[1] blue:
						   data[2] alpha:data[3] ] retain ];
		free(data);
		data = NULL;
