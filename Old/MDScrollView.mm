...
+ (id) mdScrollView
{
	return [ [ [ MDScrollView alloc ] init ] autorelease ];
}

+ (id) mdScrollViewWithFrame: (MDRect)rect background:(NSColor*)bkg
{
	return [ [ [ MDScrollView alloc ] initWithFrame:rect background:bkg ] autorelease ];
}
...
- (void) updateScroll: (double)x vertical:(double)y
{
    ...
    {
        MDRect rect = [ (MDControlView*)[ subViews objectAtIndex:z ] frame ];
        rect.x += scroll.x - prevX;
        rect.y += scroll.y - prevY;
        [ (MDControlView*)[ subViews objectAtIndex:z ] setFrame:rect ];
    }
}
...
- (void) mouseDragged:(NSEvent*)event
{
	...
	{
		[ scrollTimer invalidate ];
		scrollTimer = [ [ NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(startFade) userInfo:nil repeats:NO ] retain ]; 
	}
...
- (void) finishDraw
{
    [ super finishDraw ];
    for (int z = 0; z < [ subViews count ]; z++)
        [ [ subViews objectAtIndex:z ] finishDraw ];
}
